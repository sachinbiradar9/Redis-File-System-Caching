...
class MyRedis(redis.Redis):
    ...
    def my_set(self, key, value, hook=None):
        if hook:
            hook()
        self.set(key, value)
        self.set(key+':time_created', datetime.utcnow())

    def my_get(self, key, hook=None):
        if hook:
            hook()
        self.set(key+':last_accessed', datetime.utcnow())
        return self.get(key)

    def my_delete(self, key, hook=None):
        if hook:
            hook()
        self.delete(key)
    ...
    def my_rename(self, key, new_key, hook=None):
        if hook:
            hook()
        self.rename(key, new_key)
        self.rename(key+':last_accessed', new_key+':last_accessed')
        self.rename(key+':time_created', new_key+':time_created')
